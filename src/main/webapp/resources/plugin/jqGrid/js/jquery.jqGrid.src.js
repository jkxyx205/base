...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: "sidx",order: "sord", search:"_search",queryName:'queryName', nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				if(pN.queryName !== null) {prm[pN.queryName] = ts.p.queryName;} 
...
$.jgrid.extend({
	...
	getRowData2 : function (rowid) //add by rick
	{
	    var data = $(this).jqGrid("getGridParam", "rows");
	    var c = $(this).jqGrid("getGridParam", "colModel");
	    var key;
	    $.each(c, function (k, v)
	    {
	        if (v.key || v.name === "id") {
